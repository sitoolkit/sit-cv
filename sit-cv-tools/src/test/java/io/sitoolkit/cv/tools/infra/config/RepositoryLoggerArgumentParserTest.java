...
public class RepositoryLoggerArgumentParserTest {
    ...
    String filter1 = "include.repositoryFilter1.annotation=.*(Repository|Named);"
            + "include.repositoryFilter1.name=.*Repository.*;";

    String filter2 = "include.repositoryFilter2.annotation=;"
            + "include.repositoryFilter2.name=aaaaa;"
            + "include.repositoryFilter3.annotation=bbbbb;"
            + "include.repositoryFilter3.name=;";

    String filter3 = "exclude.repositoryFilter1.annotation=;"
    ...
    public void testParseManyFilter() throws MalformedURLException {
        RepositoryLoggerConfig config = new RepositoryLoggerArgumentParser()
            .parse(filter1 + filter2 + filter3 + base);
        ...
        assertThat(filter.getExclude().get(1).getAnnotationPattern().getPattern().toString(),
            is("ddddd"));
        assertThat(config.getRepositoryMethodMarker(), is("[RepositoryMethod]"));
    }
