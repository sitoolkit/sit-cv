    ...
    sqlPerMethodList
        ...
        .forEach(
            sqlPerMethod -> {
              ...
              if (result.isError()) {
                matrix.addError(
                    sqlPerMethod.getRepositoryMethod(),
                    sqlPerMethod.getSqlText(),
                    result.getErrMsg());
              }
        ...
        entryPointMethod -> {
          ...
          implMethods.forEach(
              methodCalledByEntryPoint -> {
                ...
                CrudRow entryPointMethodCrudRow =
                    result
                        .getCrudRowMap()
                        .computeIfAbsent(
                            entryPointMethod.getQualifiedSignature(),
                            (key) -> {
